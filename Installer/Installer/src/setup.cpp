...
bool RegisterInstalledApp(const std::wstring& installDir, const std::wstring& appPath, const std::wstring& uninstallPath) {
    ...
    ok = SetRegString(HKEY_LOCAL_MACHINE, kUninstallKey, L"Publisher", L"Visual MediaPlayer") && ok;
...
bool Uninstall() {
    ...
    const wchar_t* message = delayedSelfDelete
        ? L"Visual MediaPlayer has been uninstalled. Windows will remove the final cleanup file after the next restart."
        : L"Visual MediaPlayer has been uninstalled. A small cleanup file may remain in Program Files until it can be removed manually.";
