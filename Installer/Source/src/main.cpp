...
private:
    ...
    static constexpr ULONGLONG kPlayerControlsFadeDurationMs = 300;
    static constexpr ULONGLONG kPlayerControlsFadeMinDurationMs = 90;
    ...
    void EnterPlayerAt(double startSeconds) {
        ...
        if(playerFooterTransitionHwnd_){
            // Keep the outgoing Info/Timeline footer snapshot behind the newly visible
            // player controls.  The snapshot used to be promoted to HWND_TOP here,
            // which covered the lower player buttons while they were fading from alpha
            // zero.  When the snapshot disappeared those controls were already mostly
            // opaque, making only part of the popup appear to animate on video start.
            SetWindowPos(playerFooterTransitionHwnd_,HWND_TOP,0,0,0,0,
    ...
    void BeginControlsFade(BYTE target) {
        ...
        const ULONGLONG scaled=(kPlayerControlsFadeDurationMs*span + fullSpan-1)/fullSpan;
        controlsFadeDurationMs_=std::clamp<ULONGLONG>(scaled,kPlayerControlsFadeMinDurationMs,kPlayerControlsFadeDurationMs);
